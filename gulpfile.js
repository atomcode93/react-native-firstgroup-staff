...
gulp.task('set-onesignal:ios', (cb) => {
    const folder = path.resolve('ios/FirstStudentConnect');
...
gulp.task('set-version:ios', () => {
    const plistFile = path.resolve('ios', 'FirstStudentConnect', 'Info.plist');
...
gulp.task('set-bundle-id:ios', () => {
    const pbxProjFile = path.resolve('ios/FirstStudentConnect.xcodeproj/project.pbxproj');
...
gulp.task('clear-pods-signing', () => {
    const pbxProjFile = path.resolve('ios/Pods/Pods.xcodeproj/project.pbxproj');
    const project = xcode.project(pbxProjFile);
    project.parseSync();
    project.removeFromBuildSettings('PROVISIONING_PROFILE_SPECIFIER');
    fs.writeFileSync(pbxProjFile, project.writeSync());
});
...
gulp.task('set-fabric:ios', () => {
    return Promise.all([
        ...
        new Promise((resolve, reject) => {
            const pbxProjFolder = path.resolve('ios/FirstStudentConnect.xcodeproj');
...
gulp.task('setup-fastlane', (cb) => {
    ...
    gulp.src([path.resolve(folder, 'Appfile'), path.resolve(folder, 'Fastfile'), path.resolve(folder, 'Matchfile')])
        ...
        .pipe(replace(/@provider\s*=\s*"[^"]*"/g, `@provider = "${config.itcProvider || ''}"`))
...
gulp.task('setup:ios', (cb) => runSequence('set-version:ios', 'set-bundle-url-types:ios', 'set-onesignal:ios', 'set-bundle-id:ios', 'set-provisioning-profile', 'set-development-team', 'set-display-name:ios', 'set-fabric:ios', 'clear-pods-signing', cb));
